...
export class ProfessorCadPage {
  ...
  constructor(public navCtrl: NavController, public afAuth: AngularFireAuth, public alertCtrl: AlertController, public db: AngularFirestore) {
  }
  ...
  public async cad_professor(form: NgForm) {
    ...
    if (identificador === 'admProf2019') {
      ...
      await this.db.collection('usuarios').doc(result.user.uid).set(professor);
